<template>
  ...
  <div v-else class="flex flex-col gap-4">
    ...
    <div class="flex flex-col-reverse gap-8 xl:flex-row">
      <div v-if="bachecaRsu.length === 0" class="w-full">
      ...
      <div v-else class="flex flex-col w-full gap-4 xl:w-2/3">
        <div class="flex flex-col gap-6">
          <NewsCard v-for="post in bachecaRsu" :key="post.slug" :post="post" :vertical="false" />
        </div>
        <NPagination
...
<script setup>
  ...
  const updatePage = async (page) => {
    try {
      loading.value = true
      const filtersToRun = { categories: ['bacheca-rsu'], limit: 4, page: page, ...filters.value }
  ...
  const runSearch = async () => {
    try {
      loading.value = true
      const filtersToRun = { categories: ['bacheca-rsu'], limit: 4, page: 1, ...filters.value }
      if (filtersToRun.tags.length === 0) delete filtersToRun.tags
      if (!filtersToRun.range) delete filtersToRun.range
